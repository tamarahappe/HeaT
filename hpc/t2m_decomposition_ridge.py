...
if __name__ == "__main__":
    ...
    lat_extent_exeeded = (max(latitudes)+window) > max(stream500.latitude) or (min(latitudes)-window < min(stream500.latitude))
    lon_extent_exeeded = (max(longitudes)+window) > max(stream500.longitude) or (min(longitudes)-window < min(stream500.longitude))
    ...
    if lat_extent_exeeded or lon_extent_exeeded:
        ...
        print(f"t2m extent is lat:{(int(min(latitudes))), int(max(latitudes))}, lon:{int(min(longitudes)), int(max(longitudes))}")
    ...
    for i, lat in enumerate(latitudes):
        for j, lon in enumerate(longitudes):
            ...
            Y_, X_, penalty, t2m_mean, t2m_std, stream500_x2, GMST_x1 = preprocess_data(
                    ...
                    lower_year=lower_year, upper_year=upper_year, window=window, LENTIS_GRID=LENTIS_GRID)
    ...
    ds_out = xr.Dataset(
        ...
        coords=dict(
            longitude=("longitude", longitudes.values),
            latitude=("latitude", latitudes.values),
